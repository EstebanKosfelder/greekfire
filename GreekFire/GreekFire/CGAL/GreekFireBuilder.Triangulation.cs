   ...
   public partial class GreekFireBuilder
    {
        ...
        public void assert_valid( double time)
        {
            ...
            //      const Vertex * const u = t.vertex(cw (idx));
            //      const Vertex * const v = t.vertex(ccw(idx));
            //      const Vertex * const V = n.vertex(cw (nidx));
            //      const Vertex * const w = n.vertex(ccw(nidx));
            //      Debug.Assert(v==V);
        ...
        internal void modified(KineticTriangle t, bool front=false)
        {
        ...
        ///
        //private  void DoRawFlip(FlipEventData evnt )
        //{
        ...
        ///</summary>
        //private void DoFlip(FlipEventData evnt,  double time, bool allow_collinear = false)
        //{
           
        //    KineticTriangle n =(KineticTriangle) evnt.N;
        //    KineticTriangle t = (KineticTriangle)evnt.N;
        //    Debug.Assert(t != null);
        //    Debug.Assert(n != null);
        ...
        //internal  void DoFlipEvent(double time, KineticTriangle t, Halfedge he)
        //{
        ...
        //    Point2[] p =  he.Halfedges.Select(e=>e.Vertex.PointAt(time)).ToArray();
         
        //    double[] squared_lengths = new[]{ Mathex.squared_distance(p[1],p[2]),
        //                                      Mathex.squared_distance(p[2],p[0]),
        //                                      Mathex.squared_distance(p[0],p[1])};
                    ...
                    case 1:
                        {
                            ...
                            if (he.HasWavefrontEdge) break;



                            var ho = he.Opposite;
                            var n = (KineticTriangle)ho.Face;
                            var o = (Vertex)ho.Vertex;


                            //   is actually straight (it won't be reflex), do not flip */


                            //if (t.is_constrained(cw (reflex_idx)) && n.is_constrained(ccw(idx_in_n)) && t.vertex(ccw(reflex_idx)).is_reflex_or_straight()) break;

                            Vertex v = he.Vertex;
                            Vertex va = he.Next.Vertex;
                            Vertex vb = he.Prev.Vertex;
                            ...
                            Log($"   t:  {t}");
                            Log($"   n:  {n}");
                            Log($"   v:  {v} - {v.PointAt(time)}");
                            Log($"   va: {va} - {va.PointAt(time)}");
                            Log($"   vb: {vb} - {vb.PointAt(time)}");
                            Log($"   o : {o}  - {o.PointAt(time)}");

                            var evnt = new RefineFlipEventData(he, time);
     ...
     //   private void do_spoke_collapse_part2(Halfedge he, double time)
     //   {
     ...
     //   }
        ...
        //[TODO]
        private void do_constraint_collapse_part2(KineticTriangle t, Halfedge edge, double time)
        {


            //DBG_FUNC_BEGIN(//DBG_KT_EVENT);

            //Vertex va = t.vertices[ccw(edge_idx)];
            //Vertex vb = t.vertices[cw(edge_idx)];
            //Debug.Assert(va.has_stopped());
            //Debug.Assert(vb.has_stopped());
            //Debug.Assert(va.pos_stop() == vb.pos_stop());
            ...
            //Vertex v = vertices.make_vertex(pos, time, ea, eb);
            ////DBG(//DBG_KT) << " va: " << va;
            ////DBG(//DBG_KT) << " vb: " << vb;
        ...
        //    KineticTriangle t = KineticTriangles[evnt.t.Id];
        //    double time = evnt.Time;
        //    var edge = evnt.EventData.Es.;

        //    Debug.Assert(edge.IsConstrain);
        //    var a = edge.WavefrontEdge.GetCollapse( time, edge);
        //    Debug.Assert(evnt.EventData ==  edge.WavefrontEdge.GetCollapse( time, edge)  );
        ...
        //TODO
        //private void handle_split_event(SplitOrFlipRefineEventData evnt)
        //{
        //    //DBG_FUNC_BEGIN(//DBG_KT_EVENT);
        //    //DBG(//DBG_KT_EVENT) << evnt;

        //    Debug.Assert(evnt.Type == CollapseType.SplitOrFlipRefine);
        //    KineticTriangle  t = evnt.Triangle;
        //    double time= evnt.Time;
        //    var edge = evnt.Edge;
        //    //DBG(//DBG_KT_EVENT2) << " t:  " << &t;
        ...
        //    Vertex nva = new Vertex(this.WavefrontVertices.Count, pos, time, nea, ea, true);
        //    this.WavefrontVertices.Add(nva);
        //    Vertex nvb = new Vertex(this.WavefrontVertices.Count, pos, time, eb, neb, true);
        ...
        //     if(!(evnt.EventData is SplitEventCompare))

        //    LogIndent();
        //    Log($"{MethodBase.GetCurrentMethod()?.Name}");

        //    Debug.Assert(evntData.Type == CollapseType.SplitOrFlipRefine);

        //    KineticTriangle t = KineticTriangles[evntData.t.Id];
        //    double time = (evntData.Time);
            
        //    var edge = evntData.Edge;
        ...
        //    Debug.Assert(!vc.has_stopped());

        //    var posa = a.PointAt(time);
        //    var posb = vb.PointAt(time);
        //    var posc = vc.PointAt(time);
        //    var s = new Segment2(posa, posb);
        ...
        //    double sq_length_constraint =Mathex.squared_distance(posa,posb);
        //    double sq_length_v_to_va = Mathex.squared_distance(posc , posa);
        //    double sq_length_v_to_vb = Mathex.squared_distance(posc , posb); 
        ...
        //            if(!(sq_length_v_to_va > sq_length_constraint))
        //            {
        ...
        //            Debug.Assert(o1 != OrientationEnum.RIGHT_TURN || (vc.IsReflexOrStraight && o0 != OrientationEnum.RIGHT_TURN));

        //            //TODO  ckeck
        //            flipEdge = evntData.RelevantEdge.Prev;
        //        }
        ...
        //            if(!(sq_length_v_to_vb > sq_length_constraint))
        //            {
        //                  Debug.Assert(sq_length_v_to_vb > sq_length_constraint); /* Check that v to vb is the longest spoke */
        //            }
        //            Debug.Assert(a == edge.Next.Vertex);
        //    //        Debug.Assert(va.IsReflexOrStraight);

        //            /* If we come from behind, we don't really care about the first of these things in the discunjtion,
        //             * if we face it head on, we don't care about the second.  hmm. */
        //            Debug.Assert(o0 != OrientationEnum.LEFT_TURN || (vc.IsReflexOrStraight && o1 != OrientationEnum.LEFT_TURN));
        //            //TODO  ckeck
                 
        //            flipEdge = evntData.RelevantEdge.Next;
        //        };
        //        var vertexMovesOverSpokeEventData = new VertexMovesOverSpokeEventData(flipEdge, longest_spoke, time);
        //        var c = t.RefineCollapseSpec(vertexMovesOverSpokeEventData );
        //        Log($"Refining to {c}");
        ...
        //    {
        //        Log($"CASE 2: pos == posa");
        //        var spokeEventData = new SpokeCollapseEventData(edge.Prev, time);
        //        var c = t.RefineCollapseSpec(spokeEventData);
        ...
        //    {
        //        Log($"CASE 2: pos == posb");
        //        var spokeEventData = new SpokeCollapseEventData(edge.Next, time);
        //        var c = t.RefineCollapseSpec(spokeEventData);
        ...
        //    var evntData = (FlipEventData)evnt.EventData;
        //      DoFlipEvent(evnt.Time, t, evnt.EventData.E.RelevantEdge);
        ...
        private void handle_face_with_infintely_fast_opposing_vertex(CollapseEvent evnt)
        {
            ...
            //    Vertex v = t.vertices[t_fast_idx];
            ...
            //    /* =============================== */
            //    Vertex v_cw = t.vertices[cw(vidx_in_tc)];
            //    Vertex v_ccw = t.vertices[ccw(vidx_in_tc)];
        ...
        private void handle_face_with_infintely_fast_weighted_vertex(CollapseEvent evnt)
        {
            ...
            //Vertex* o = losing_edge.vertex(1-winning_edge_idx_in_v);
