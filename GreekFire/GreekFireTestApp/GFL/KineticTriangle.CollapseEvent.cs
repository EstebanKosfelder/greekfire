...
namespace GFL
{
    ...
    public partial class KineticTriangle
    {
        ...
        public CollapseSpec GetCollapse(double time_now)
        {
            Debug.Assert(!IsDead);
            Debug.Assert(!IsDying);
            if (!collapse_spec_valid)
            {
                collapse_spec = ComputeCollapse(time_now);
        ...
        internal CollapseSpec RefineCollapseSpec(CollapseSpec c)
        {
            Debug.Assert(collapse_spec_valid);
            Debug.Assert(collapse_spec.allows_refinement_to(c));
            
            collapse_spec.Assign(c);
            return collapse_spec;
        }

        private CollapseSpec GetCachedCollapse()
        {
            Debug.Assert(!IsDying);
            Debug.Assert(collapse_spec_valid);
            return collapse_spec;
        ...
        internal void InvalidateCollapseSpec()
        {
            Debug.Assert(!IsDead);
            collapse_spec_valid = false;
        }

        private CollapseSpec ComputeCollapse(double time_now)
        {
            Log("");
        ...
        private CollapseSpec ComputeCollapseBounded(double time_now)
        { 
        ...
        /// XXX do meet events.
        ///</summary>
        private CollapseSpec ComputeCollapseBoundedConstrained0(double time_now)
        { // {{{
        ...
        private CollapseSpec ComputeFlipEvent(double time_now, Polynomial1D determinant)
        { // {{{
          //DBG_FUNC_BEGIN(//DBG_TRIANGLE);
          //DBG(//DBG_TRIANGLE) << this;
            ...
            if (!could_flip)
            {
                result = EventThatWillNotHappen(component, time_now, determinant);
            }
            else
            {
                ...
                switch (result.Type)
                {
                    ...
                    case CollapseType.VertexMovesOverSpoke:
                        Debug.Assert(!result.RelevantEdge.IsConstrain);
                        if (result.RelevantEdge.Vertex.IsNotConvex)
                        {
                        ...
                        else
                        {
                           ...
                           // result = new CollapseSpec(component, CollapseType.InvalidEvent, result.Time);
                            //#else
        ...
        ///</summary>
        private CollapseSpec ComputeCollapseBoundedConstrained1(double time_now)
        {
            Log("");
            Log($"{MethodBase.GetCurrentMethod()?.Name}");
            LogIndent();
           ...
           // var he = this.Halfedges.Where(he => he.IsConstrain).First();
            ...
            KineticHalfedge wfHe = Halfedges.Where(he => he.IsConstrain).First();
            ...
            if (wf.ParallelEndpoints(time_now))
            {
                ...
                if (edge_collapse.Type == EdgeCollapseType.Always)
                {
                    ...
                    result = new CollapseSpec(component, edge_collapse, wfHe);
                }
            ...
            {
                CollapseSpec candidate = wf.GetCollapse(component, time_now, wfHe);
        ...
        private CollapseSpec ComputeSplitOrFlipEventBoundedConstrained1(double time_now, KineticHalfedge wfHe, Polynomial1D determinant)
        {
                ...
                switch (vertex_on_line_type)
                {
                    case VertexOnSupportingLineType.Once:
                        if (collapse_time > time_now)
                        {
                            ...
                            result = new CollapseSpec(component, CollapseType.SplitOrFlipRefine, collapse_time,he);
                        }
                    ...
                    case VertexOnSupportingLineType.Always:
                        ...
                        result = new CollapseSpec(component, CollapseType.SplitOrFlipRefine, time_now, he);
                        break;
                }
            }
        ...
        ///</summary>
        private CollapseSpec ComputeCollapseBoundedConstrained2(double time_now)
        { // {{{
          //DBG_FUNC_BEGIN(//DBG_TRIANGLE);
          //DBG(//DBG_TRIANGLE) << this;
            CollapseSpec result = new CollapseSpec(component);

            KineticHalfedge[] wavefrontsEdges = this.Halfedges.Where(h => h.IsConstrain).ToArray();
            ...
            CollapseSpec c1 = (wavefrontsEdges[0].WavefrontEdge.GetCollapse(component, time_now, wavefrontsEdges[0]));
            CollapseSpec c2 = (wavefrontsEdges[1].WavefrontEdge.GetCollapse(component, time_now, wavefrontsEdges[1]));
            ...
            { /* both constraints collapse at this time. */
                result = new CollapseSpec(component, CollapseType.TriangleCollapse, c1.Time);
            }
            else
            {
        ...
        ///</summary>
        private CollapseSpec ComputeCollapseBoundedConstrained3(double time_now)
        { // {{{
          //DBG_FUNC_BEGIN(//DBG_TRIANGLE);
          //DBG(//DBG_TRIANGLE) << this;
            ...
            CollapseSpec candidate = (hes[0].WavefrontEdge.GetCollapse(component, time_now, hes[0]));
            for (int i = 1; i < 3; ++i)
            {
                Debug.Assert(candidate == hes[i].WavefrontEdge.GetCollapse(component, time_now, hes[i]));
            };
            Debug.Assert(candidate.Type == CollapseType.ConstraintCollapse);
            CollapseSpec result = new CollapseSpec(component, CollapseType.TriangleCollapse, candidate.Time);
