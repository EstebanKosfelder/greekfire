...
namespace GFL
{
    ...
    public class HeapItemBase<PriorityType> where PriorityType : class, IComparable<PriorityType>
    ...
    public class HeapBase<PriorityType, HeapItemBase> where PriorityType : class, IComparable<PriorityType>
    {
        private List<HeapItemBase<PriorityType>> v_;
        ...
        {
            Debug.Assert(a >= 0 && a < size());
            Debug.Assert(b >= 0 && b < size());
        ...
        {
            Debug.Assert(root_idx >= 0 && root_idx < size());
            ...
            while (true)
            {
                ...
                { /* so, root only has one child. */
                    /*STATS_STMT(*/
                    { if (lp == orig_root.priority) ++heap_eq_ctr; }/*)*/;
                    if (lp.CompareTo(orig_root.priority) ==(int) ESign.SMALLER)
                    {
                        smallest = left;
                    }
                    else
                ...
                else
                {
                    ...
                    {
                        /* STATS_STMT(*/
                        { if (lp == rp) ++heap_eq_ctr; }
                        //);
                        smallest = (lp.CompareTo(rp) <= (int)ESign.ZERO) ? left : right;
                    }
                    else
                    {
                        /* STATS_STMT(*/
        ...
        private void sift_up(int child_idx)
        {
            ...
            Debug.Assert(child_idx >= 0 && child_idx < size());
            ...
            while (new_child_idx != 0)
            {
                ...
                /*STATS_STMT(*/
                {
                    if (v_[parent].priority.CompareTo(orig_child.priority) == 0) ++heap_eq_ctr;
                }
                /*);*/
        ...
        public void fix_idx(int idx)
        {
            ...
            {
                /*STATS_STMT(*/
                { if (v_[idx].priority == v_[parent_idx(idx)].priority) ++heap_eq_ctr; }/*);*/
                sift_up(idx);
            }
            else
            {
        ...
        public HeapItemBase<PriorityType> drop_element(HeapItemBase<PriorityType> e)
        {
    ...
    public class EventQueueItem : HeapItemBase<CollapseEvent>
    {
        ...
        public void update_priority(double now)
        {
    ...
    {
        private List<KineticTriangle> need_update;
        private List<KineticTriangle> need_dropping;
        ...
        public void process_pending_updates(double now)
        {
        ...
        public void needs_update(KineticTriangle t, bool may_have_valid_collapse_spec = false)
        {
            // DBG_FUNC_BEGIN(DBG_EVENTQ);
            // DBG(DBG_EVENTQ) << "t" << t;
