...
namespace GFL
{
    ...
    public struct Interval
    {
       ...
       public IEnumerable<Vector2D> GetCorners(Vector2D min, Vector2D max)
        {
            yield return new Vector2D(min.X, min.Y); // bottom-left
            yield return new Vector2D(max.X, min.Y); // bottom-right
            yield return new Vector2D(max.X, max.Y); // top-right
            yield return new Vector2D(min.X, max.Y); // top-left
        }
        ...
        public bool AreNear(Interval b, double eps = Const.EPSILON)
        {
        ...
        public bool Contains(double val, double eps = Const.EPSILON)
        {
            return (Min.AreNear(val, eps) || Min < val) &&
                (Max.AreNear(val, eps) || val < Max);
        }

        public bool Contains(Interval val, double eps = Const.EPSILON)
        {
        ...
        public bool InteriorContains(double val, double eps = Const.EPSILON) =>
            Min < val && val < Max && !Min.AreNear(val, eps) && !Max.AreNear(val, eps);

        public bool InteriorContains(Interval val, double eps = Const.EPSILON) =>
            Min < val.Min && val.Max < Max && !Min.AreNear(val.Min) && !Max.AreNear(val.Max, eps);

        public bool InteriorIntersects(Interval val, double eps = Const.EPSILON)
        {
        ...
        public bool Intersects(Interval other, double eps = Const.EPSILON)
        {
        ...
        public bool LowerContains(double val, double eps = Const.EPSILON)
        { return (Min <= val || Min.AreNear(val, eps)) && val < Max && !Max.AreNear(val, eps); }

        public bool LowerContains(Interval val, double eps = Const.EPSILON)
        ...
        public bool UpperContains(double val, double eps = Const.EPSILON)
        { return Min < val && !Min.AreNear(val, eps) && (val <= Max || Max.AreNear(val, eps)); }

        public bool UpperContains(Interval val, double eps = Const.EPSILON)
        { return Min < val.Min && !Min.AreNear(val.Min, eps) && (val.Max <= Max || Max.AreNear(val.Max, eps)); }
    ...
    public struct Rect2D  //:IObjectBounds
	{
		...
		public static Rect2D FromOriginAndSize(Vector2D origin, Vector2D size){
		...
		public Rect2D (IEnumerable<Vector2D> items){
			axis = new Interval[] {Interval.NaN(),Interval.NaN(), Interval.NaN() };
			foreach (var i in items)
			{
				Extend(i);
        ...
        */
        ...
        public IEnumerable<Vector2D> GetCorners()
        {
            yield return new Vector2D(Left, Top); // bottom-left
            yield return new Vector2D(Right, Top); // bottom-right
            yield return new Vector2D(Right, Bottom); // top-right
            yield return new Vector2D(Right, Bottom); // top-left
        }
        public Rect2D(Vector2D value):this ( new Interval(value.X),new Interval(value.Y)){}
		...
		public static implicit operator Rect2D(Vector2D p){
			return new Rect2D(p);
		}
		...
		public Vector2D Min
		{
			get
			{
				return new Vector2D(axis[0].Min,axis[1].Min);
			}
			set
			{
				for (int i = 0; i < 2; i++)
		...
		public Vector2D Max
		{
			get
			{
				return new Vector2D(axis[0].Max,axis[1].Max);
			}
			set
			{
				for (int i = 0; i < 2; i++)
		...
		public Vector2D Center { get { return (Size / 2) + Min; } }
		public Vector2D Size { get { return (Max - Min); } }
		...
		public Vector2D Clamp(Vector2D p){
			return new Vector2D (axis [0].Clamp (p.X), axis [1].Clamp (p.Y));
		}
        ...
        */
		public void Extend(IEnumerable<Vector2D> items)
		{
			foreach (var o in items){
				Extend(o);
			}
        ...
        public void Extend(Vector2D o)
        {

            for (int i = 0; i < 2; i++)
                axis[i].Extend(o[i]);
        }
        public IEnumerable<Vector2D> Extrems(){


			for ( int x = 0 ; x < 2; x++){
				for ( int y = 0 ; y <2; y++){
					
						yield return new Vector2D (axis [0] [x], axis [1] [y]);
		...
		public Rect2D Offset(Vector2D value)
		{
        ...
        /*
		public bool Intersects(IObjectBounds other, double eps = Const.EPSILON){
			return Intersects(other.Bounds,eps);
		}
        */

        public bool Intersects(Rect2D other,double eps = Const.EPSILON )
		{
        ...
        /*
		public bool Contains(IObjectBounds other, double eps = Const.EPSILON){
			return Contains(other.Bounds,eps);
		}
        */
		public bool Contains(Rect2D other, double eps = Const.EPSILON)
		{
			if (IsNaN() || other.IsNaN())
				throw new Exception("NaN Bounds");
			for (int i = 0; i < 2; i++)
		...
		public bool Contains(Vector2D point, double eps = Const.EPSILON)
		{
		...
		public bool InteriorContains(Rect2D other, double eps = Const.EPSILON)
		{
			if (IsNaN() || other.IsNaN())
				throw new Exception("NaN Bounds");
			for (int i = 0; i < 2; i++)
		...
		#region == != Operator
		public static bool operator != (Rect2D A, Vector2D B)
		{
			return !A.Equals (B);
		}
		...
		public static Rect2D operator + (Rect2D A, Vector2D B)
		{
